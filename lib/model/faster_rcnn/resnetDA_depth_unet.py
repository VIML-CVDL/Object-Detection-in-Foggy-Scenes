...
class resnetDA_depth_unet(_fasterRCNN):
...
#                                                up_conv_in_channels=128, up_conv_out_channels=64))
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes):
    ...
    #depth = self.den(base_feat)

    #base_feat = torch.div(base_feat - torch.min(base_feat), torch.max(base_feat) - torch.min(base_feat))

    #num_fts = depth.size()[1]
    #for num_ft in range(num_fts):
#        cmap = plt.get_cmap('jet')
#        feat = torch.FloatTensor(cmap(depth[0,num_ft:num_ft+1,:,:].detach().cpu().numpy())).permute(0,3,1,2)
#        savename = '../../depth_'+str(num_ft)+'.png'
#        torchvision.utils.save_image(feat, savename)
    base_feat_down_3 = self.RCNN_base[:3](im_data)
    base_feat_down_4 = self.RCNN_base[3](base_feat_down_3)
    base_feat_down_5 = self.RCNN_base[4](base_feat_down_4)
    base_feat_down_6= self.RCNN_base[5](base_feat_down_5)
    base_feat= self.RCNN_base[6](base_feat_down_6)
    depth = self.den(base_feat)
    #base_feat = self.RCNN_base(im_data)
    ...
    #up_1 = self.up_blocks[0](base_feat)
    #up_2 = self.up_blocks[1](up_1)
    #rec_feat = self.up_blocks[2](up_2)
    base_feat_down_6_empty = torch.zeros_like(base_feat_down_6)
    base_feat_down_5_empty = torch.zeros_like(base_feat_down_5)
    base_feat_down_4_empty = torch.zeros_like(base_feat_down_4)
    base_feat_down_3_empty = torch.zeros_like(base_feat_down_3)

    up_1 = self.up_blocks[0](base_feat, base_feat_down_6.detach())
    up_2 = self.up_blocks[1](up_1, base_feat_down_5.detach())
    up_3 = self.up_blocks[2](up_2, base_feat_down_4.detach(), no_up=True)
    up_4 = self.up_blocks[3](up_3, base_feat_down_3.detach())
...
#        torchvision.utils.save_image(feat, savename)
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, depth#, depth
