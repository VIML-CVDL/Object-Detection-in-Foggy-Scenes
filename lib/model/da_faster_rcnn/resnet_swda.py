...
class resnet_swda(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
            tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes, tgt_need_backprop,eta=1.0):
    ...
    if self.lc:
        ...
        d_pixel = self.netD_pixel(grad_reverse(base_feat1))
    base_feat = self.RCNN_base2(base_feat1)
    if self.gc:
        domain_p, _ = self.netD(grad_reverse(base_feat))
        _,feat = self.netD(base_feat.detach())
    else:
        domain_p = self.netD(grad_reverse(base_feat))
    ...
    if self.lc:
        tgt_d_pixel, _ = self.netD_pixel(grad_reverse(tgt_base_feat1))
        _,tgt_feat_pixel = self.netD_pixel(tgt_base_feat1.detach())
    else:
        tgt_d_pixel = self.netD_pixel(grad_reverse(tgt_base_feat1))
    tgt_base_feat = self.RCNN_base2(tgt_base_feat1)
    if self.gc:
        tgt_domain_p, _ = self.netD(grad_reverse(tgt_base_feat))
        _,tgt_feat = self.netD(tgt_base_feat.detach())
    else:
        tgt_domain_p = self.netD(grad_reverse(tgt_base_feat))
    ...
    domain_t = Variable(torch.ones(tgt_domain_p.size(0)).long().cuda())
    dloss_t = 0.5 * FL(tgt_domain_p, domain_t)
    # local alignment loss
    dloss_t_p = 0.5 * torch.mean((1 - tgt_d_pixel) ** 2)

    DA_img_loss_cls_gc = dloss_s + dloss_t
    DA_img_loss_cls_lc = dloss_s_p + dloss_t_p

    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           DA_img_loss_cls_gc, DA_img_loss_cls_lc
