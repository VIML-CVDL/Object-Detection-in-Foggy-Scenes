...
class resnetDA_transmission_r(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    rloss = nn.MSELoss()
    reg_loss = -0.1*rloss(tgt_prior, transmission_map)

    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss, reg_loss
