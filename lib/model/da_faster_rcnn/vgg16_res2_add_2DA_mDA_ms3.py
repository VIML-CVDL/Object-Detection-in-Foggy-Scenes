...
class vgg16_res2_add_2DA_mDA_ms3(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_imageDAres = _ImageDA(self.dout_base_model)
    self.RCNN_imageDA_1 = _ImageDA(128)
    self.RCNN_imageDAres_1 = _ImageDA(128)
    self.RCNN_imageDA_2 = _ImageDA(256)
    self.RCNN_imageDAres_2 = _ImageDA(256)
    self.RCNN_imageDA_3 = _ImageDA(self.dout_base_model)
    self.RCNN_imageDAres_3 = _ImageDA(self.dout_base_model)
    self.RCNN_instanceDAres = _InstanceDA()
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    '''----------stage 3------------------------'''
    base_feat_rgb_3 = self.RCNN_base3(base_feat_rgb_2)
    res_feat_3 = self.RCNN_base3(res_feat_2)

    '''----------stage 4------------------------'''
    base_feat_rgb = self.RCNN_base4(base_feat_rgb_3)
    res_feat = self.RCNN_base4(res_feat_3)
    ...
    '''----------stage 3------------------------'''
    tgt_base_feat_rgb_3 = self.RCNN_base3(tgt_base_feat_rgb_2)
    tgt_res_feat_3 = self.RCNN_base3(tgt_res_feat_2)

    '''----------stage 4------------------------'''
    tgt_base_feat_rgb = self.RCNN_base4(tgt_base_feat_rgb_3)
    tgt_res_feat = self.RCNN_base4(tgt_res_feat_3)
    ...
    base_score_1, base_label_1 = self.RCNN_imageDA_1(base_feat_rgb_1, need_backprop)
    base_score_2, base_label_2 = self.RCNN_imageDA_2(base_feat_rgb_2, need_backprop)
    base_score_3, base_label_3 = self.RCNN_imageDA_3(base_feat_rgb_3, need_backprop)
    base_score, base_label = self.RCNN_imageDA(base_feat_rgb, need_backprop)

    # Image DA
    base_prob_1 = F.log_softmax(base_score_1, dim=1)
    DA_img_loss_cls_1 = F.nll_loss(base_prob_1, base_label_1)
    base_prob_2 = F.log_softmax(base_score_2, dim=1)
    DA_img_loss_cls_2 = F.nll_loss(base_prob_2, base_label_2)
    base_prob_3 = F.log_softmax(base_score_3, dim=1)
    DA_img_loss_cls_3 = F.nll_loss(base_prob_3, base_label_3)
    ...
    """  ************** taget loss ****************  """
    tgt_base_score_1, tgt_base_label_1 = \
        self.RCNN_imageDA_1(tgt_base_feat_rgb_1, tgt_need_backprop)
    tgt_base_score_2, tgt_base_label_2 = \
        self.RCNN_imageDA_2(tgt_base_feat_rgb_2, tgt_need_backprop)
    tgt_base_score_3, tgt_base_label_3 = \
        self.RCNN_imageDA_3(tgt_base_feat_rgb_3, tgt_need_backprop)
    tgt_base_score, tgt_base_label = \
        self.RCNN_imageDA(tgt_base_feat_rgb, tgt_need_backprop)

    # Image DA
    tgt_base_prob_1 = F.log_softmax(tgt_base_score_1, dim=1)
    tgt_DA_img_loss_cls_1 = F.nll_loss(tgt_base_prob_1, tgt_base_label_1)
    tgt_base_prob_2 = F.log_softmax(tgt_base_score_2, dim=1)
    tgt_DA_img_loss_cls_2 = F.nll_loss(tgt_base_prob_2, tgt_base_label_2)
    tgt_base_prob_3 = F.log_softmax(tgt_base_score_3, dim=1)
    tgt_DA_img_loss_cls_3 = F.nll_loss(tgt_base_prob_3, tgt_base_label_3)
    ...
    # DA LOSS
    DA_res_img_loss_cls = 0
    DA_res_ins_loss_cls = 0

    tgt_DA_res_img_loss_cls = 0
    tgt_DA_res_ins_loss_cls = 0

    base_score_res_1, base_label_res_1 = self.RCNN_imageDAres_1(res_feat_1, need_backprop)
    base_score_res_2, base_label_res_2 = self.RCNN_imageDAres_2(res_feat_2, need_backprop)
    base_score_res_3, base_label_res_3 = self.RCNN_imageDAres_3(res_feat_3, need_backprop)
    base_score_res, base_label_res = self.RCNN_imageDAres(res_feat, need_backprop)

    # Image DA
    base_prob_res_1 = F.log_softmax(base_score_res_1, dim=1)
    DA_res_img_loss_cls_1 = F.nll_loss(base_prob_res_1, base_label_res_1)
    base_prob_res_2 = F.log_softmax(base_score_res_2, dim=1)
    DA_res_img_loss_cls_2 = F.nll_loss(base_prob_res_2, base_label_res_2)
    base_prob_res_3 = F.log_softmax(base_score_res_3, dim=1)
    DA_res_img_loss_cls_3 = F.nll_loss(base_prob_res_3, base_label_res_3)
    base_prob_res = F.log_softmax(base_score_res, dim=1)
    DA_res_img_loss_cls = F.nll_loss(base_prob_res, base_label_res)

    instance_sigmoid_res, same_size_label_res = self.RCNN_instanceDAres(pooled_res_feat, need_backprop)
    instance_res_loss = nn.BCELoss()
    DA_res_ins_loss_cls = instance_res_loss(instance_sigmoid_res, same_size_label_res)

    #consistency_prob = torch.max(F.softmax(base_score, dim=1),dim=1)[0]
    consistency_prob_res = F.softmax(base_score_res, dim=1)[:,1,:,:]
    consistency_prob_res=torch.mean(consistency_prob_res)
    consistency_prob_res=consistency_prob_res.repeat(instance_sigmoid_res.size())

    DA_res_cst_loss=self.consistency_loss(instance_sigmoid_res,consistency_prob_res.detach())

    """  ************** taget loss ****************  """
    tgt_base_score_res_1, tgt_base_label_res_1 = \
        self.RCNN_imageDAres_1(tgt_res_feat_1, tgt_need_backprop)
    tgt_base_score_res_2, tgt_base_label_res_2 = \
        self.RCNN_imageDAres_2(tgt_res_feat_2, tgt_need_backprop)
    tgt_base_score_res_3, tgt_base_label_res_3 = \
        self.RCNN_imageDAres_3(tgt_res_feat_3, tgt_need_backprop)
    tgt_base_score_res, tgt_base_label_res = \
        self.RCNN_imageDAres(tgt_res_feat, tgt_need_backprop)

    # Image DA
    tgt_base_prob_res_1 = F.log_softmax(tgt_base_score_res_1, dim=1)
    tgt_DA_res_img_loss_cls_1 = F.nll_loss(tgt_base_prob_res_1, tgt_base_label_res_1)
    tgt_base_prob_res_2 = F.log_softmax(tgt_base_score_res_2, dim=1)
    tgt_DA_res_img_loss_cls_2 = F.nll_loss(tgt_base_prob_res_2, tgt_base_label_res_2)
    tgt_base_prob_res_3 = F.log_softmax(tgt_base_score_res_3, dim=1)
    tgt_DA_res_img_loss_cls_3 = F.nll_loss(tgt_base_prob_res_3, tgt_base_label_res_3)
    tgt_base_prob_res = F.log_softmax(tgt_base_score_res, dim=1)
    tgt_DA_res_img_loss_cls = F.nll_loss(tgt_base_prob_res, tgt_base_label_res)


    tgt_instance_sigmoid_res, tgt_same_size_label_res = \
        self.RCNN_instanceDAres(tgt_pooled_res_feat, tgt_need_backprop)
    tgt_instance_res_loss = nn.BCELoss()

    tgt_DA_res_ins_loss_cls = \
        tgt_instance_res_loss(tgt_instance_sigmoid_res, tgt_same_size_label_res)


    tgt_consistency_prob_res = F.softmax(tgt_base_score_res, dim=1)[:, 0, :, :]
    tgt_consistency_prob_res = torch.mean(tgt_consistency_prob_res)
    tgt_consistency_prob_res = tgt_consistency_prob_res.repeat(tgt_instance_sigmoid_res.size())

    tgt_DA_res_cst_loss = self.consistency_loss(tgt_instance_sigmoid_res, tgt_consistency_prob_res.detach())

    """  ************** multi-DAs' reg loss ****************  """
    mDA_cst_loss=self.consistency_loss(instance_sigmoid_res,consistency_prob.detach())
    tgt_mDA_cst_loss = self.consistency_loss(tgt_instance_sigmoid_res, tgt_consistency_prob.detach())

    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           DA_img_loss_cls,DA_ins_loss_cls,tgt_DA_img_loss_cls,tgt_DA_ins_loss_cls,DA_cst_loss,tgt_DA_cst_loss,\
           DA_res_img_loss_cls,DA_res_ins_loss_cls,tgt_DA_res_img_loss_cls,tgt_DA_res_ins_loss_cls,DA_res_cst_loss,\
           tgt_DA_res_cst_loss,mDA_cst_loss,tgt_mDA_cst_loss, DA_img_loss_cls_1,tgt_DA_img_loss_cls_1, DA_res_img_loss_cls_1, \
           tgt_DA_res_img_loss_cls_1, DA_img_loss_cls_2,tgt_DA_img_loss_cls_2, DA_res_img_loss_cls_2, \
           tgt_DA_res_img_loss_cls_2, DA_img_loss_cls_3,tgt_DA_img_loss_cls_3, DA_res_img_loss_cls_3, \
           tgt_DA_res_img_loss_cls_3
