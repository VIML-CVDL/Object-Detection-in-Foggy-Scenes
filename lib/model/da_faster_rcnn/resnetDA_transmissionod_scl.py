...
class resnetDA_transmissionod_scl(_fasterRCNN):
  ...
  def _init_weights(self):
    ...
    normal_init(self.transfer, 0, 0.01, cfg.TRAIN.TRUNCATED)
  ...
  def _init_modules(self):
    ...
    self.pen = PEN()
    self.transfer = nn.Linear(2208, 2048)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    pooled_feat = torch.cat((scl_feat, pooled_feat), 0)
    pooled_feat = self.fuse(pooled_feat.permute(1,0))
    pooled_feat = pooled_feat.permute(1,0)
