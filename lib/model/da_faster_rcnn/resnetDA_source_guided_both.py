...
class resnetDA_source_guided_both(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    guided_loss_pooled = nn.MSELoss()

    source_guided_loss = guided_loss_pooled(base_feat, base_feat_teacher.detach())
    ...
    if cfg.POOLING_MODE == 'crop':
        ...
        pooled_feat_teacher = self.teacher.RCNN_roi_align(base_feat_teacher, rois.view(-1, 5))
    elif cfg.POOLING_MODE == 'pool':
        pooled_feat = self.RCNN_roi_pool(base_feat, rois.view(-1,5))

    # feed pooled features to top model
    pooled_feat = self._head_to_tail(pooled_feat)
    pooled_feat_teacher = self.teacher._head_to_tail(pooled_feat_teacher)

    source_guided_loss += guided_loss_pooled(pooled_feat, pooled_feat_teacher.detach())
    # compute bbox offset
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           DA_img_loss_cls,DA_ins_loss_cls,tgt_DA_img_loss_cls,tgt_DA_ins_loss_cls,DA_cst_loss,tgt_DA_cst_loss, source_guided_loss#, self.selected_out
