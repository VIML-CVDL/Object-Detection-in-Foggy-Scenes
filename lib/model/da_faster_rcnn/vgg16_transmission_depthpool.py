...
class vgg16_transmission_depthpool(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    # not using the last maxpool layer
    self.RCNN_cls_score = nn.Linear(4145, self.n_classes)

    if self.class_agnostic:
      self.RCNN_bbox_pred = nn.Linear(4145, 4)
    else:
      self.RCNN_bbox_pred = nn.Linear(4145, 4 * self.n_classes)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    pooled_dep = pooled_dep.mean(1).view(256,-1)
