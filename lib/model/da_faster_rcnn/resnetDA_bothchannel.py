...
class resnetDA_bothchannel(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.se3 = SELayer(self.dout_base_model, 16)

    # Smooth layers
    self.RCNN_smooth1 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth2 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth3 = nn.Conv2d(1024, 1024, kernel_size=3, stride=1, padding=1)
    ...
    if cfg.RESNET.FIXED_BLOCKS >= 1:
      for p in self.RCNN_base1.parameters(): p.requires_grad=False
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    base_feat_1_se = self.se1(base_feat_1)
    base_feat_1_comb = base_feat_1 + base_feat_1_se
    base_feat_1_comb = self.RCNN_smooth1(base_feat_1_comb)

    '''----------stage 2------------------------'''
    base_feat_2 = self.RCNN_base2(base_feat_1_comb)
    base_feat_2_se = self.se2(base_feat_2)
    base_feat_2_comb = base_feat_2 + base_feat_2_se
    base_feat_2_comb = self.RCNN_smooth2(base_feat_2_comb)

    '''----------stage 3------------------------'''
    base_feat_tmp = self.RCNN_base3(base_feat_2_comb)
    base_feat_se = self.se3(base_feat_tmp)
    base_feat = base_feat_tmp + base_feat_se
    base_feat = self.RCNN_smooth3(base_feat)
    ...
    tgt_base_feat_1_se = self.se1(tgt_base_feat_1)
    tgt_base_feat_1_comb = tgt_base_feat_1 + tgt_base_feat_1_se
    tgt_base_feat_1_comb = self.RCNN_smooth1(tgt_base_feat_1_comb)

    '''----------stage 2------------------------'''
    tgt_base_feat_2 = self.RCNN_base2(tgt_base_feat_1_comb)
    tgt_base_feat_2_se = self.se2(tgt_base_feat_2)
    tgt_base_feat_2_comb = tgt_base_feat_2 + tgt_base_feat_2_se
    tgt_base_feat_2_comb = self.RCNN_smooth2(tgt_base_feat_2_comb)

    '''----------stage 3------------------------'''
    tgt_base_feat_tmp = self.RCNN_base3(tgt_base_feat_2_comb)
    tgt_base_feat_se = self.se3(tgt_base_feat_tmp)
    tgt_base_feat = tgt_base_feat_tmp + tgt_base_feat_se
    tgt_base_feat = self.RCNN_smooth3(tgt_base_feat)
