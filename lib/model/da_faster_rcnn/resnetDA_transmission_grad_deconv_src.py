...
class EdgeDetection(nn.Module):
    def __init__(self):
        ...
        self.deconv1 = torch.nn.ConvTranspose2d(1024, 512, 3, stride=(2,2), padding=1)
        self.deconv2 = torch.nn.ConvTranspose2d(512, 256, 3, stride=(2,2), padding=1)
        self.deconv3 = torch.nn.ConvTranspose2d(256, 128, 3, stride=(2,2), padding=1)
        self.deconv4 = torch.nn.ConvTranspose2d(128, 64, 3, padding=1)
        self.deconv5 = torch.nn.ConvTranspose2d(64, 3, 3, padding=1)
        self._init_weights()
    ...
    def _init_weights(self):
      ...
      normal_init(self.deconv4, 0, 0.01)
      normal_init(self.deconv5, 0, 0.01)
...
class resnetDA_transmission_grad_deconv_src(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
            tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes, tgt_need_backprop, transmission_map=None, tgt_transmission_map=None, grad=None, tgt_grad=None):
    ...
    edge = self.edgeDetection(base_feat)
    dloss = nn.MSELoss()
    grad_loss = dloss(edge, grad)



    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss, grad_loss
