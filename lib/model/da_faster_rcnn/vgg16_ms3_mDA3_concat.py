...
class vgg16_ms3_mDA3_concat(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_imageDA_res = _ImageDA(self.dout_base_model)
    self.RCNN_imageDA_res1 = _ImageDA(128)
    self.RCNN_imageDA_res2 = _ImageDA(256)
    self.RCNN_imageDA_res3 = _ImageDA(self.dout_base_model)
    # Fix the layers before conv3:
    for layer in range(5):
      for p in self.RCNN_base1[layer].parameters(): p.requires_grad = False

    # self.RCNN_base = _RCNN_base(vgg.features, self.classes, self.dout_base_model)
    self.mergeres = MergeRes(1024, 512)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    res_feat_1 = torch.reshape(res_tensor, (b, c, h, w))
    ...
    base_feat = torch.cat((base_feat, res_feat), 1)
    base_feat = self.mergeres(base_feat)
    ...
    tgt_blue = torch.cat((tgt_blue, tgt_im_data[:, 2:3, :, :]), 1)
    tgt_base_feat_red = self.RCNN_base1(tgt_red)
    tgt_base_feat_green = self.RCNN_base1(tgt_green)
    tgt_base_feat_blue = self.RCNN_base1(tgt_blue)
    ...
    tgt_res_feat_1 = torch.reshape(tgt_res_tensor, (b, c, h, w))

    '''----------stage 2------------------------'''
    tgt_base_feat_2= self.RCNN_base2(tgt_base_feat_1)
    tgt_res_feat_2= self.RCNN_base2(tgt_res_feat_1)

    '''----------stage 3------------------------'''
    tgt_base_feat_3= self.RCNN_base3(tgt_base_feat_2)
    tgt_res_feat_3= self.RCNN_base3(tgt_res_feat_2)

    '''----------stage 4------------------------'''
    tgt_base_feat= self.RCNN_base4(tgt_base_feat_3)
    tgt_res_feat= self.RCNN_base4(tgt_res_feat_3)
    tgt_base_feat = torch.cat((tgt_base_feat, tgt_res_feat), 1)
    tgt_base_feat = self.mergeres(tgt_base_feat)
    ...
    base_score_3, base_label_3 = self.RCNN_imageDA_3(base_feat_3, need_backprop)
    base_score_2, base_label_2 = self.RCNN_imageDA_2(base_feat_2, need_backprop)
    base_score_1, base_label_1 = self.RCNN_imageDA_1(base_feat_1, need_backprop)
    ...
    tgt_base_score_3, tgt_base_label_3 = \
        self.RCNN_imageDA_3(tgt_base_feat_3, tgt_need_backprop)
    tgt_base_score_2, tgt_base_label_2 = \
        self.RCNN_imageDA_2(tgt_base_feat_2, tgt_need_backprop)
    tgt_base_score_1, tgt_base_label_1 = \
        self.RCNN_imageDA_1(tgt_base_feat_1, tgt_need_backprop)
    ...
    base_score_res_3, base_label_res_3 = self.RCNN_imageDA_res3(res_feat_3, need_backprop)
    base_score_res_2, base_label_res_2 = self.RCNN_imageDA_res2(res_feat_2, need_backprop)
    base_score_res_1, base_label_res_1 = self.RCNN_imageDA_res1(res_feat_1, need_backprop)
    base_score_res, base_label_res = self.RCNN_imageDA_res(res_feat, need_backprop)
    ...
    tgt_base_score_res_3, tgt_base_label_res_3 = \
        self.RCNN_imageDA_res3(tgt_res_feat_3, tgt_need_backprop)
    tgt_base_score_res_2, tgt_base_label_res_2 = \
        self.RCNN_imageDA_res2(tgt_res_feat_2, tgt_need_backprop)
    tgt_base_score_res_1, tgt_base_label_res_1 = \
        self.RCNN_imageDA_res1(tgt_res_feat_1, tgt_need_backprop)
    tgt_base_score_res, tgt_base_label_res = \
        self.RCNN_imageDA_res(tgt_res_feat, tgt_need_backprop)
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           ...
           DA_img_loss_cls_1,tgt_DA_img_loss_cls_1,DA_img_loss_cls_2,tgt_DA_img_loss_cls_2,DA_img_loss_cls_3,tgt_DA_img_loss_cls_3,\
           DA_res_img_loss_cls,tgt_DA_res_img_loss_cls,DA_res_img_loss_cls_1,tgt_DA_res_img_loss_cls_1,\
           DA_res_img_loss_cls_2,tgt_DA_res_img_loss_cls_2,DA_res_img_loss_cls_3,tgt_DA_res_img_loss_cls_3
