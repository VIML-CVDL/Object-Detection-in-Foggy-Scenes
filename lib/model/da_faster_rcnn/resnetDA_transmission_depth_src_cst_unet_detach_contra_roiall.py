...
class UpBlock(nn.Module):
    ...
    def forward(self, up_x, down_x=None, no_up=False):
        ...
        _,_,h,w = x.size()
        if h == 76:
            x = torch.nn.Upsample(size=(75, 150), mode='bilinear')(x)
...
class resnetDA_transmission_depth_src_cst_unet_detach_contra_roiall(_fasterRCNN):
...
#                                                up_conv_in_channels=128, up_conv_out_channels=64))
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    no_rois = tgt_rois.size()
    rec_size = rec_feat.size()
    roi_masks = torch.ones(no_rois[1], rec_size[1], rec_size[2], rec_size[3])
    for i in range(no_rois[1]):
        tmp_roi = torch.floor(tgt_rois[:,i,:])
        x1 = int(tmp_roi[0, 1].item())
        y1 = int(tmp_roi[0, 2].item())
        x2 = int(tmp_roi[0, 3].item())
        y2 = int(tmp_roi[0, 4].item())
        if x1 == x2 and y1==y2:
            roi_masks[i,:,y1,x1] += 1.0
        elif x1 == x2:
            roi_masks[i,:,y1:y2,x1] += 1.0
        elif y1 == y2:
            roi_masks[i,:,y1,x1:x2] += 1.0
        else:
            roi_masks[i,:,y1:y2,x1:x2] += 1.0

    recloss = nn.MSELoss()
    roi_masks = roi_masks / 10
    roi_masks = torch.sum(roi_masks, 0, True)
    rec_loss = recloss(rec_feat*roi_masks.cuda(), tgt_new*roi_masks.cuda()) / no_rois[1]

    #tgt_im_new = (tgt_im_data-torch.min(tgt_im_data))/(torch.max(tgt_im_data)-torch.min(tgt_im_data))
    #rec_loss_tgt = recloss(rec_feat*roi_masks.cuda(), tgt_im_new*roi_masks.cuda()) / no_rois[1]
    #contra_loss = rec_loss/rec_loss_tgt
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss, depth_loss, cst_loss, rec_loss#/2, contra_loss/10
