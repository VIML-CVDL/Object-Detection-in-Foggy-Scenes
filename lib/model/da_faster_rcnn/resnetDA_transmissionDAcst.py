...
class Pixel2Label(nn.Module):
    def __init__(self):
        super(Pixel2Label,self).__init__()
        self.LabelResizeLayer=ImageLabelResizeLayer()

    def forward(self,x,need_backprop):
        x = F.upsample(x, size=(38,75), mode='bilinear')
        label=self.LabelResizeLayer(x,need_backprop)
        return x[:,0:2,:,:],label

class resnetDA_transmissionDAcst(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    prior_base_score, prior_base_label = self.p2l(prior.detach(), need_backprop)
    tgt_prior_base_score, tgt_prior_base_label = self.p2l(tgt_prior.detach(), tgt_need_backprop)

    prior_cst_loss = self.consistency_loss(base_score, prior_base_score.detach())
    tgt_prior_cst_loss = self.consistency_loss(tgt_base_score, tgt_prior_base_score.detach())
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss,\
           ...
           prior_cst_loss/(38*75*2), tgt_prior_cst_loss/(38*75*2)
