...
class resnetDA_transmission_depth_src_cst_unet_detach_contra_roi(_fasterRCNN):
...
#                                                up_conv_in_channels=128, up_conv_out_channels=64))
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    recloss = nn.MSELoss()
    roi_masks = torch.mean(roi_masks, 0, True)
    rec_loss = recloss(rec_feat*roi_masks.cuda(), tgt_new*roi_masks.cuda())
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss, depth_loss, cst_loss, rec_loss#, contra_loss/10
