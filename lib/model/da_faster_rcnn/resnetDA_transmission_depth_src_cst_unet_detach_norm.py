...
class resnetDA_transmission_depth_src_cst_unet_detach_norm(_fasterRCNN):
...
#                                                up_conv_in_channels=128, up_conv_out_channels=64))
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    log_tgt_prior = -torch.log(tgt_prior)
    norm_log_tgt_prior = (log_tgt_prior-torch.min(log_tgt_prior))/(torch.max(log_tgt_prior)-torch.min(log_tgt_prior)+0.000001)
    norm_tgt_dep = (tgt_dep-torch.min(tgt_dep))/(torch.max(tgt_dep)-torch.min(tgt_dep)+0.000001)
    cstloss = nn.MSELoss()
    #cst_loss = cstloss(-torch.log(tgt_prior), 1-tgt_dep.detach())
    cst_loss = cstloss(norm_log_tgt_prior, norm_tgt_dep.detach())
