...
class UpBlock(nn.Module):
    ...
    def forward(self, up_x, down_x=None, no_up=False):
        ...
        if down_x is None:
            ...
            _,_,h1,w1 = x.size()
            if h1>600:
                x = torch.nn.Upsample(size=(600, 1200), mode='bilinear')(x)
            return x
        if no_up:
            x = up_x
        else:
...
class resnetDA_transmission_depth_src_cst_unetmultirec(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    up_blocks1 = []
    up_blocks1.append(UpBlock(1024, 256))
    up_blocks1.append(UpBlock(256, 64))
    up_blocks1.append(UpBlock(64, 3))

    up_blocks2 = []
    up_blocks2.append(UpBlock(512, 256))
    up_blocks2.append(UpBlock(256, 64))
    up_blocks2.append(UpBlock(64, 3))
...
#                                                up_conv_in_channels=128, up_conv_out_channels=64))
    ...
    self.up_blocks1 = nn.ModuleList(up_blocks1)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    base_feat_2= self.RCNN_base[:6](im_data)
    base_feat = self.RCNN_base[6](base_feat_2)
    prior = self.pen(base_feat)

    up_1 = self.up_blocks1[0](base_feat)
    up_1 = self.up_blocks1[1](up_1)
    rec_feat1 = self.up_blocks1[2](up_1)

    up_2 = self.up_blocks2[0](base_feat_2)
    up_2 = self.up_blocks2[1](up_2)
    rec_feat2 = self.up_blocks2[2](up_2)

    im_data_new = (im_data-torch.min(im_data))/(torch.max(im_data)-torch.min(im_data))

    b,c,h2,w2 = rec_feat2.size()
    im_data_new_up2 = im_data_new.clone()
    im_data_new_up2 = im_data_new_up2.resize_(b,c,h2,w2)

    #tgt_clean_new_up2 = torch.nn.Upsample(size=(h2, w2), mode='bilinear')(tgt_clean_new)
    b,c,h1,w1 = rec_feat1.size()
    im_data_new_up1 = im_data_new.clone()
    im_data_new_up1 = im_data_new_up1.resize_(b,c,h1,w1)

    recloss = nn.MSELoss()
    rec_loss1 = recloss(rec_feat1.detach(), im_data_new_up1)
    rec_loss2 = recloss(rec_feat2.detach(), im_data_new_up2)
    ...
    # feed image data to base model to obtain base feature map
    tgt_base_feat_2= self.RCNN_base[:6](tgt_im_data)
    tgt_base_feat = self.RCNN_base[6](tgt_base_feat_2)
    tgt_prior = self.pen(tgt_base_feat)

    with torch.no_grad():
        tgt_up_1 = self.up_blocks1[0](tgt_base_feat)
        tgt_up_1 = self.up_blocks1[1](tgt_up_1)
        tgt_rec_feat1 = self.up_blocks1[2](tgt_up_1)

        tgt_up_2 = self.up_blocks2[0](tgt_base_feat_2)
        tgt_up_2 = self.up_blocks2[1](tgt_up_2)
        tgt_rec_feat2 = self.up_blocks2[2](tgt_up_2)

    tgt_clean_new = (tgt_clean-torch.min(tgt_clean))/(torch.max(tgt_clean)-torch.min(tgt_clean))
    ...
    b,c,h2,w2 = tgt_rec_feat2.size()
    tgt_clean_new_up2 = tgt_clean_new.clone()
    tgt_clean_new_up2 = tgt_clean_new_up2.resize_(b,c,h2,w2)

    #tgt_clean_new_up2 = torch.nn.Upsample(size=(h2, w2), mode='bilinear')(tgt_clean_new)
    b,c,h1,w1 = tgt_rec_feat1.size()
    tgt_clean_new_up1 = tgt_clean_new.clone()
    tgt_clean_new_up1 = tgt_clean_new_up1.resize_(b,c,h1,w1)

    #tgt_clean_new_up1 = torch.nn.Upsample(size=(h1, w1), mode='bilinear')(tgt_clean_new)
    #rec_loss = 0
    tgt_rec_loss1 = recloss(tgt_rec_feat1, tgt_clean_new_up1)
    tgt_rec_loss2 = recloss(tgt_rec_feat2, tgt_clean_new_up2)

    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss, depth_loss, cst_loss, rec_loss1, rec_loss2, tgt_rec_loss1, tgt_rec_loss2
