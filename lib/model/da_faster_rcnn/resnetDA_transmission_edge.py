...
def get_edge(x):
    ...
    kernalx = torch.from_numpy(kx).float().cuda()
    kernalx = kernalx.repeat(1,c,1,1)
    delta_x = F.sigmoid(F.conv2d(x,kernalx,padding=1))
    ky = np.array([[[[1,2,1],[0,0,0],[-1,-2,-1]]]])
    kernaly = torch.from_numpy(ky).float().cuda()
    kernaly = kernaly.repeat(1,c,1,1)
    delta_y = F.sigmoid(F.conv2d(x,kernaly,padding=1))
    G = torch.sqrt(torch.pow(delta_x,2)+ torch.pow(delta_y,2))
    return G

class resnetDA_transmission_edge(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    img_hazy_edge = get_edge(tgt_base_feat)
    img_clean_edge = get_edge(base_feat)
    edge_loss = torch.abs(1 - img_clean_edge.sum() / img_hazy_edge.sum())
