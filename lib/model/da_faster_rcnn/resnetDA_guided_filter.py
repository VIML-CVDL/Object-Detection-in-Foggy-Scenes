...
class resnetDA_guided_filter(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    '''----------stage 1-----------------------'''
    # feed image data to base model to obtain base feature map
    LF, HF = decomposition(im_data)
    base_feat = self.RCNN_base1(im_data)
    base_feat_1_LF = self.RCNN_base1(LF)
    base_feat_1_HF = self.RCNN_base1(HF)

    base_feat_1 = torch.cat((base_feat, base_feat_1_LF), 1)
    base_feat_1 = torch.cat((base_feat_1, base_feat_1_HF), 1)
    base_feat_1 = self.se1(base_feat_1)
    base_feat = base_feat + base_feat_1
    ...
    # feed image data to base model to obtain base feature map
    '''----------stage 1-----------------------'''
    tgt_LF, tgt_HF = decomposition(tgt_im_data)
    tgt_base_feat = self.RCNN_base1(tgt_im_data)
    tgt_base_feat_1_LF = self.RCNN_base1(tgt_LF)
    tgt_base_feat_1_HF = self.RCNN_base1(tgt_HF)

    tgt_base_feat_1 = torch.cat((tgt_base_feat, tgt_base_feat_1_LF), 1)
    tgt_base_feat_1 = torch.cat((tgt_base_feat_1, tgt_base_feat_1_HF), 1)
    tgt_base_feat_1 = self.se1(tgt_base_feat_1)
    tgt_base_feat = tgt_base_feat + tgt_base_feat_1
    tgt_base_feat = self.relu(tgt_base_feat)

    '''----------stage 2------------------------'''
    tgt_base_feat = self.RCNN_base2(tgt_base_feat)
