...
class resnetDA_dis(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop, ref_im_data,#im_data_ref, im_info_ref, gt_boxes_ref, num_boxes_ref,
    ...
    base_feat_ref = self.RCNN_base(ref_im_data)
    feat_p = self.proj(base_feat)
    ref_feat_p = self.proj(base_feat_ref)
    ...
    tgt_feat_p = self.proj(tgt_base_feat)
    ...
    """ref loss"""
    ref_loss = nn.L1Loss()
    loss_ref = ref_loss(feat_p, ref_feat_p.detach())
    distance = torch.dist(feat_p, ref_feat_p)
    projection = (feat_p - ref_feat_p) / distance
    norm = torch.dot((feat_p - tgt_feat_p), projection)

    norm_loss = nn.L1Loss()
    loss_norm = norm_loss(norm, tgt_need_backprop)
