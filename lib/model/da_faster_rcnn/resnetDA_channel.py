...
class resnetDA_channel(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.relu = nn.ReLU(inplace=True)
    self.se = SELayer(self.dout_base_model, 16)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    base_feat_1 = self.RCNN_base(im_data)
    base_feat = self.se(base_feat_1)
    ...
    # feed image data to base model to obtain base feature map
    tgt_base_feat_1 = self.RCNN_base(tgt_im_data)
    tgt_base_feat = self.se(tgt_base_feat_1)
