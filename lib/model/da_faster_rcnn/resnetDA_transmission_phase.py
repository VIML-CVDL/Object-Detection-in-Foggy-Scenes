...
class SMOOTH(nn.Module):
    def __init__(self):
        ...
        self.smooth_fuse = conv1x1(3072, 1024)
        self._init_weights()
    ...
    def _init_weights(self):
      def normal_init(m, mean, stddev, truncated=False):
        ...
        if truncated:
          ...
          m.weight.data.normal_(mean, stddev)
          #m.bias.data.zero_()
      normal_init(self.smooth_fuse, 0, 0.01)

    def forward(self, x):
        x = F.relu(self.smooth_fuse(x))
        return x
...
def decomposition(x):
    ...
    min_v = torch.min(HF)
    range_v = torch.max(HF) - min_v
    HF = (HF - min_v) / range_v

    return HF

class resnetDA_transmission_phase(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    hp = decomposition(im_data)

    # feed image data to base model to obtain base feature map
    base_feat = self.RCNN_base(im_data)
    hp_feat_1 = self.RCNN_base(hp[:,0:3,:,:])
    hp_feat_2 = self.RCNN_base(hp[:,3:,:,:])
    prior = self.pen(base_feat)
    hp_base_fuse = torch.cat((base_feat, hp_feat_1, hp_feat_2), 1)
    hp_base_fuse = self.smooth_fuse(hp_base_fuse)

    # feed base feature map tp RPN to obtain rois
    self.RCNN_rpn.train()
    rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(hp_base_fuse, im_info, gt_boxes, num_boxes)
    ...
    if cfg.POOLING_MODE == 'crop':
        ...
        pooled_feat = self.RCNN_roi_align(hp_base_fuse, rois.view(-1, 5))
