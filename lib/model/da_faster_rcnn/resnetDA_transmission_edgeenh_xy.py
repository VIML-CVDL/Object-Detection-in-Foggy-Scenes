...
class resnetDA_transmission_edgeenh_xy(_fasterRCNN):
  ...
  def grad_conv_x(self, x):

    b,c,h,w = x.size()
    k = np.array([[[[1,0,-1],[2,0,-2],[1,0,-1]]]])
    kernal = torch.from_numpy(k).float().cuda()
    kernal = kernal.repeat(1,c,1,1)
    delta_x = F.conv2d(x[:,:,:,:],kernal,padding=1)

    return  F.sigmoid(delta_x)

  def grad_conv_y(self, y):

    b,c,h,w = y.size()
    k = np.array([[[[1,2,1],[0,0,0],[-1,-2,-1]]]])
    kernal = torch.from_numpy(k).float().cuda()
    kernal = kernal.repeat(1,c,1,1)
    delta_y = F.conv2d(y[:,:,:,:],kernal,padding=1)

    return  F.sigmoid(delta_y)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    img_clean_edge_x = self.grad_conv_x(base_feat)
    img_hazy_edge_x = self.grad_conv_x(tgt_base_feat)

    b,c,h,w = base_feat.size()
    edge_loss_x = torch.abs(1 - ((img_clean_edge_x*img_hazy_edge_x).sum()/c) / (img_hazy_edge_x.sum()/c*10))

    img_clean_edge_y = self.grad_conv_y(base_feat)
    img_hazy_edge_y = self.grad_conv_y(tgt_base_feat)

    b,c,h,w = base_feat.size()
    edge_loss_y = torch.abs(1 - ((img_clean_edge_y*img_hazy_edge_y).sum()/c) / (img_hazy_edge_y.sum()/c*10))
    edge_loss = (edge_loss_x + edge_loss_y) / 20
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss, edge_loss
