...
class vgg16_res2_add_2DA_mDA_ms_rpnclsfocal(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    self.RCNN_rpn.train()
    rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(base_feat, im_info, gt_boxes, num_boxes, focal=True)
    ...
    if self.training:
        # classification loss
        FL = FocalLoss(class_num=self.n_classes, gamma=2)
