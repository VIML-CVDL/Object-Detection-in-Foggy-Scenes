...
class vgg16_ms3_skip(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_base1 = nn.Sequential(*list(vgg.features._modules.values())[:-27])
    self.RCNN_base1_pool = nn.Sequential(*list(vgg.features._modules.values())[-27:-25])
    self.RCNN_base2 = nn.Sequential(*list(vgg.features._modules.values())[-25:-22])
    self.RCNN_base2_pool = nn.Sequential(*list(vgg.features._modules.values())[-22:-20])
    self.RCNN_base3 = nn.Sequential(*list(vgg.features._modules.values())[-20:-15])
    self.RCNN_base3_pool = nn.Sequential(*list(vgg.features._modules.values())[-15:-13])
    self.RCNN_base4 = nn.Sequential(*list(vgg.features._modules.values())[-13:-8])
    self.RCNN_base4_pool = nn.Sequential(*list(vgg.features._modules.values())[-8:-6])
    self.RCNN_base5 = nn.Sequential(*list(vgg.features._modules.values())[-6:-1])
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    base_feat_1_pooled = self.RCNN_base1_pool(base_feat_1)

    '''----------stage 2------------------------'''
    base_feat_2 = self.RCNN_base2(base_feat_1_pooled)
    base_feat_2 = base_feat_2 + base_feat_1_pooled
    base_feat_2_pooled = self.RCNN_base2_pool(base_feat_2)

    '''----------stage 3------------------------'''
    base_feat_3 = self.RCNN_base3(base_feat_2_pooled)
    base_feat_3 = base_feat_3 + base_feat_2_pooled
    base_feat_3_pooled = self.RCNN_base3_pool(base_feat_3)

    '''----------stage 4------------------------'''
    base_feat_4 = self.RCNN_base4(base_feat_3_pooled)
    base_feat_4 = base_feat_4 + base_feat_3_pooled
    base_feat_4_pooled = self.RCNN_base4_pool(base_feat_4)

    '''----------stage 5------------------------'''
    base_feat = self.RCNN_base5(base_feat_4_pooled)
    base_feat = base_feat + base_feat_4_pooled
    ...
    tgt_base_feat_1_pooled = self.RCNN_base1_pool(tgt_base_feat_1)

    '''----------stage 2------------------------'''
    tgt_base_feat_2= self.RCNN_base2(tgt_base_feat_1_pooled)
    tgt_base_feat_2 = tgt_base_feat_2 + tgt_base_feat_1_pooled
    tgt_base_feat_2_pooled = self.RCNN_base2_pool(tgt_base_feat_2)

    '''----------stage 3------------------------'''
    tgt_base_feat_3= self.RCNN_base3(tgt_base_feat_2_pooled)
    tgt_base_feat_3 = tgt_base_feat_3 + tgt_base_feat_2_pooled
    tgt_base_feat_3_pooled = self.RCNN_base3_pool(tgt_base_feat_3)

    '''----------stage 4------------------------'''
    tgt_base_feat_4= self.RCNN_base4(tgt_base_feat_3_pooled)
    tgt_base_feat_4 = tgt_base_feat_4 + tgt_base_feat_3_pooled
    tgt_base_feat_4_pooled = self.RCNN_base4_pool(tgt_base_feat_4)

    '''----------stage 5------------------------'''
    tgt_base_feat= self.RCNN_base5(tgt_base_feat_4_pooled)
    tgt_base_feat = tgt_base_feat + tgt_base_feat_4_pooled
    ...
    base_score_3, base_label_3 = self.RCNN_imageDA_3(base_feat_4, need_backprop)
    base_score_2, base_label_2 = self.RCNN_imageDA_2(base_feat_3, need_backprop)
    base_score_1, base_label_1 = self.RCNN_imageDA_1(base_feat_2, need_backprop)
    ...
    tgt_base_score_3, tgt_base_label_3 = \
        self.RCNN_imageDA_3(tgt_base_feat_4, tgt_need_backprop)
    tgt_base_score_2, tgt_base_label_2 = \
        self.RCNN_imageDA_2(tgt_base_feat_3, tgt_need_backprop)
    tgt_base_score_1, tgt_base_label_1 = \
        self.RCNN_imageDA_1(tgt_base_feat_2, tgt_need_backprop)
