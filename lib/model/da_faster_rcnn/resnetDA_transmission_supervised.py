...
class resnetDA_transmission(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    if self.training:
        # classification loss
        RCNN_loss_cls += F.cross_entropy(tgt_cls_score, tgt_rois_label)

        # bounding box regression L1 loss
        RCNN_loss_bbox += _smooth_l1_loss(tgt_bbox_pred, tgt_rois_target, tgt_rois_inside_ws, tgt_rois_outside_ws)

    ploss = nn.MSELoss()
    prior_loss = ploss(prior, transmission_map)
    prior_loss += ploss(tgt_prior, tgt_transmission_map)

    rpn_loss_cls += tgt_rpn_loss_cls
    rpn_loss_bbox += tgt_rpn_loss_bbox
