...
class resnet_transmission_depth_src_cst_unet_detach_p(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    up_blocks.append(UpBlock(256+64, 128))
    up_blocks.append(UpBlock(128, 64))
    up_blocks.append(UpBlock(64, 3))
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop=None,
            tgt_im_data=None, tgt_im_info=None, tgt_gt_boxes=None, tgt_num_boxes=None, tgt_need_backprop=None,
            transmission_map=None, tgt_transmission_map=None, depth=None, tgt_depth=None, tgt_clean=None, teacher=False, da=True):


    tgt_prior_for_loss = None
    tgt_prior_for_loss = Variable(tgt_prior_for_loss)
    if teacher:
            self.training = False
    ...
    # feed image data to base model to obtain base feature map
    tgt_base_feat_down_3 = self.RCNN_base[:3](tgt_im_data)
    tgt_base_feat_down_4 = self.RCNN_base[3](tgt_base_feat_down_3)
    tgt_base_feat_down_5 = self.RCNN_base[4](tgt_base_feat_down_4)
    tgt_base_feat_down_6= self.RCNN_base[5](tgt_base_feat_down_5)
    tgt_base_feat = self.RCNN_base[6](tgt_base_feat_down_6)
    tgt_prior = self.pen(tgt_base_feat)

    up_1 = self.up_blocks[0](tgt_base_feat, tgt_base_feat_down_6.detach())
    up_2 = self.up_blocks[1](up_1, tgt_base_feat_down_5.detach())
    up_3 = self.up_blocks[2](up_2, tgt_base_feat_down_4.detach(), no_up=True)
    up_4 = self.up_blocks[3](up_3, tgt_base_feat_down_3.detach())
    rec_feat = self.up_blocks[4](up_4)

    tgt_new = (tgt_clean-torch.min(tgt_clean))/(torch.max(tgt_clean)-torch.min(tgt_clean))

    ploss = nn.MSELoss()
    _,_,h1,w1 = prior.size()
    _,_,h,w = transmission_map.size()
    if h1 != h:
        transmission_map = torch.nn.Upsample(size=(h1, w1), mode='bilinear')(transmission_map)
    if w1 != w:
        transmission_map = torch.nn.Upsample(size=(h1, w1), mode='bilinear')(transmission_map)
    prior_loss = ploss(prior, transmission_map)


    _,_,h1,w1 = tgt_prior.size()
    _,_,h,w,_ = tgt_transmission_map.size()
    flag = 0
    if h1 != h:
        tgt_prior_for_loss = torch.nn.Upsample(size=(h1, w1), mode='bilinear')(tgt_transmission_map.squeeze(0).permute([0,3,1,2]))
        flag = 1
    if w1 != w:
        tgt_prior_for_loss = torch.nn.Upsample(size=(h1, w1), mode='bilinear')(tgt_transmission_map.squeeze(0).permute([0,3,1,2]))
        flag = 1
    if flag == 0:
        ...
        prior_loss += ploss(tgt_prior, tgt_prior_for_loss)
    ...
    _,_,h,w = depth.size()
    if h1 != h:
        depth = torch.nn.Upsample(size=(h1, w1), mode='bilinear')(depth)
    if w1 != w:
        depth = torch.nn.Upsample(size=(h1, w1), mode='bilinear')(depth)
    ...
    _,_,h1,w1 = rec_feat.size()
    _,_,h,w = tgt_new.size()
    if h1 != h:
        rec_feat = torch.nn.Upsample(size=(h, w), mode='bilinear')(rec_feat)
    if w1 != w:
        rec_feat = torch.nn.Upsample(size=(h, w), mode='bilinear')(rec_feat)
    recloss = nn.MSELoss()
    ...
    #DA_img_loss_cls,DA_ins_loss_cls,tgt_DA_img_loss_cls,tgt_DA_ins_loss_cls,DA_cst_loss,tgt_DA_cst_loss#, tgt_rpn_loss_cls, tgt_RCNN_loss_cls#, self.selected_out
