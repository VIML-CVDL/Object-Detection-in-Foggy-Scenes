...
def make_layer(block, ch, planes, blocks, stride=1):
    downsample = None
    if stride != 1 or ch != planes * block.expansion:
      downsample = nn.Sequential(
        nn.Conv2d(ch, planes * block.expansion,
              kernel_size=1, stride=stride, bias=False),
        nn.BatchNorm2d(planes * block.expansion),
      )

    layers = []
    layers.append(block(ch, planes, stride, downsample))
...
class resnetDA_transmission_scl_mr(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    pooled_feat_1 = pooled_feat_1.mean(3).mean(2).view(128,-1)
    pooled_feat_2 = pooled_feat_2.mean(3).mean(2).view(128,-1)
