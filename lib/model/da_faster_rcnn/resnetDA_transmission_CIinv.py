...
class resnetDA_transmission_CIinv(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    ci_feat = ci_feat.repeat(1, 3, 1, 1)
    base_feat = self.RCNN_base(ci_feat)
    ...
    # feed image data to base model to obtain base feature map
    tgt_ci_feat = self.ciconv(tgt_im_data)
    tgt_ci_feat = tgt_ci_feat.repeat(1, 3, 1, 1)
    tgt_base_feat = self.RCNN_base(tgt_ci_feat)
