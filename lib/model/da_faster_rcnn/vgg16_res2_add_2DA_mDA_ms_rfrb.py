...
class RFRB2(nn.Module):
    def __init__(self, context=False):
        super(RFRB2, self).__init__()
        self.pool1 = nn.MaxPool2d(2, stride=2)
        self.conv1 = conv3x3(512, 512, padding=1)
        self.conv2 = conv3x3(512, 512, padding=1)
        self.conv3 = conv3x3(512, 512, padding=1)
        self.context = context
...
class vgg16_res2_add_2DA_mDA_RFRB(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_base1 = nn.Sequential(*list(vgg.features._modules.values())[:-8])
    self.RCNN_base2 = nn.Sequential(*list(vgg.features._modules.values())[-8:-1])
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    base_feat_rgb = self.RCNN_base2(base_feat_rgb)
    red = im_data[:, 0:1, :, :]
    ...
    base_feat_red = self.RCNN_base2(base_feat_red)
    base_feat_green = self.RCNN_base1(green)
    base_feat_green = self.RCNN_base2(base_feat_green)
    base_feat_blue = self.RCNN_base1(blue)
    base_feat_blue = self.RCNN_base2(base_feat_blue)
    ...
    # feed image data to base model to obtain base feature map
    tgt_base_feat_rgb = self.RCNN_base1(tgt_im_data)
    restored_feat = self.RFRB2(tgt_base_feat_rgb)
    tgt_base_feat_rgb = self.RCNN_base2(tgt_base_feat_rgb)
    tgt_base_feat_rgb = torch.add(tgt_base_feat_rgb, restored_feat)
    ...
    restored_feat_red = self.RFRB2(tgt_base_feat_red)
    tgt_base_feat_red = self.RCNN_base2(tgt_base_feat_red)
    tgt_base_feat_red = torch.add(tgt_base_feat_red, restored_feat_red)

    tgt_base_feat_green = self.RCNN_base1(tgt_green)
    restored_feat_green = self.RFRB2(tgt_base_feat_green)
    tgt_base_feat_green = self.RCNN_base2(tgt_base_feat_green)
    tgt_base_feat_green = torch.add(tgt_base_feat_green, restored_feat_green)

    tgt_base_feat_blue = self.RCNN_base1(tgt_blue)
    restored_feat_blue = self.RFRB2(tgt_base_feat_blue)
    tgt_base_feat_blue = self.RCNN_base2(tgt_base_feat_blue)
    tgt_base_feat_blue = torch.add(tgt_base_feat_blue, restored_feat_blue)
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           ...
           tgt_DA_res_cst_loss,mDA_cst_loss,tgt_mDA_cst_loss
