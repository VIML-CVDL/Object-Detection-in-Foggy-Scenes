...
class PEN(nn.Module):
    ...
    def forward(self, x, grad_rev=True):
        if grad_rev:
            x = grad_reverse(x)
        x = self.conv1(x)
        x = F.relu(self.bn1(x))
...
class resnetDA_transmissionodnew(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    prior_transfer = self.pen(base_feat.detach(), grad_rev=False)
    prior_transfer = torch.sum(prior_transfer, 1) / 3
    prior_transfer = prior_transfer.view(1, -1).repeat(pooled_feat.size(0), 1)
    prior_transfer = self.transfer(prior_transfer)
    pooled_feat = torch.cat((prior_transfer, pooled_feat), 1)
