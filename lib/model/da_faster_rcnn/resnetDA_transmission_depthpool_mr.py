...
class resnetDA_transmission_depthpool_mr(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_cls_score = nn.Linear(2048+49+49+49+49, self.n_classes)
    if self.class_agnostic:
      self.RCNN_bbox_pred = nn.Linear(2048+49+49+49+49, 4)
    else:
      self.RCNN_bbox_pred = nn.Linear(2048+49+49+49+49, 4 * self.n_classes)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    feat1 = self.netD_forward1(base_feat_1.detach())
    base_feat_2 = self.RCNN_base[5](base_feat_1)
    feat2 = self.netD_forward2(base_feat_2.detach())
    ...
    if cfg.POOLING_MODE == 'crop':
        ...
        pooled_dep = self.RCNN_roi_align(dep, rois.view(-1, 5))
        pooled_feat1 = self.RCNN_roi_align(feat1, rois.view(-1, 5))
        pooled_feat2 = self.RCNN_roi_align(feat2, rois.view(-1, 5))
        pooled_feat3 = self.RCNN_roi_align(feat3, rois.view(-1, 5))
    elif cfg.POOLING_MODE == 'pool':
        pooled_feat = self.RCNN_roi_pool(base_feat, rois.view(-1,5))

    # feed pooled features to top model
    pooled_feat = self._head_to_tail(pooled_feat)
    pooled_dep = pooled_dep.mean(1).view(128,-1)
    pooled_feat1 = pooled_feat1.mean(1).view(128,-1)
    pooled_feat2 = pooled_feat2.mean(1).view(128,-1)
    pooled_feat3 = pooled_feat3.mean(1).view(128,-1)
    pooled_feat = torch.cat((pooled_feat1, pooled_feat2, pooled_feat3, pooled_feat), 1)
    pooled_feat = torch.cat((pooled_dep.detach(), pooled_feat), 1)
