...
class resnet_pixel(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.Squeeze = squeeze(512+1024, 1024)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    base_feat = F.upsample(input=base_feat, size=(h,w), mode='bilinear', align_corners=True)
    base_feat_det = torch.cat((base_feat, base_feat_1),1)
    base_feat_det = self.Squeeze(base_feat_det)
    ...
    tgt_base_feat = F.upsample(input=tgt_base_feat, size=(h,w), mode='bilinear', align_corners=True)
    tgt_base_feat_det = torch.cat((tgt_base_feat, tgt_base_feat_1),1)
    tgt_base_feat_det = self.Squeeze(tgt_base_feat_det)

    # feed base feature map tp RPN to obtain rois
    self.RCNN_rpn.eval()
    tgt_rois, tgt_rpn_loss_cls, tgt_rpn_loss_bbox = \
        self.RCNN_rpn(tgt_base_feat_det, tgt_im_info, tgt_gt_boxes, tgt_num_boxes)
