...
from model.utils.bbox_utils import bbox_overlaps
...
class resnet_pseudo_uncertainty(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop=None,
            tgt_im_data=None, tgt_im_info=None, tgt_gt_boxes=None, tgt_num_boxes=None, tgt_need_backprop=None, teacher=False):
    ...
    cls_prob = cls_prob.view(batch_size, rois.size(1), -1)
    bbox_pred = bbox_pred.view(batch_size, rois.size(1), -1)
    _,num_samples,_ = rois.size()
    pos = torch.argmax(cls_prob, dim=2)
    pos = pos[0]
    print(bbox_pred)
    print(gt_boxes)
    jomi
    ...
    for idx in range(num_samples):
        if pos[idx] > 0:
            bbox_pred_tmp = bbox_pred.squeeze(0)
            pos_bboxes = bbox_pred_tmp[idx].unsqueeze(0)
            gt_bboxes = rois_target[idx].unsqueeze(0)
