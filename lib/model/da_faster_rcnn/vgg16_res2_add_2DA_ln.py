...
class vgg16_res2_add_2DA_ln(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    max_feat = torch.reshape(max_tensor, (b, c, h, w))
    min_feat = torch.reshape(min_tensor, (b, c, h, w))
    mul_max_feat = self.Mul_MAX(max_feat)
    mul_min_feat = self.Mul_MIN(min_feat)
    ...
    res_feat = self.Mul_CONTRAST(res_feat)
    ...
    tgt_max_feat = torch.reshape(tgt_max_tensor, (b, c, h, w))
    tgt_min_feat = torch.reshape(tgt_min_tensor, (b, c, h, w))
    tgt_mul_max_feat = self.Mul_MAX(tgt_max_feat)
    tgt_mul_min_feat = self.Mul_MIN(tgt_min_feat)
    ...
    tgt_base_feat = self.Mul_CONTRAST(tgt_res_feat)+tgt_base_feat_rgb
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           DA_img_loss_cls,DA_ins_loss_cls,tgt_DA_img_loss_cls,tgt_DA_ins_loss_cls,DA_cst_loss,tgt_DA_cst_loss,DA_res_img_loss_cls,DA_res_ins_loss_cls,tgt_DA_res_img_loss_cls,tgt_DA_res_ins_loss_cls,DA_res_cst_loss,tgt_DA_res_cst_loss
