...
class resnetDA_transmission_depth_src_cst_unet_detach_depthmask(_fasterRCNN):
...
#                                                up_conv_in_channels=128, up_conv_out_channels=64))
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    tmp_tgt_dep = torch.nn.functional.upsample(tgt_dep.detach(), size=[600,1200], mode='bilinear', align_corners=None)
    ...
    recloss = nn.MSELoss()
    depth_rec_feat = rec_feat * (1-tmp_tgt_dep)
    depth_tgt_new = tgt_new * (1-tmp_tgt_dep)
    rec_loss = recloss(depth_rec_feat, depth_tgt_new)
    #rec_loss = 0
