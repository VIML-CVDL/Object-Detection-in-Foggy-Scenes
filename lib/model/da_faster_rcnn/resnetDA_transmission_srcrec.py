...
class resnetDA_transmission_srcrec(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    im_data_down = F.upsample(im_data, mode='bilinear', size=(32,69))
    im_data_down = im_data_down/255

    # feed image data to base model to obtain base feature map
    base_feat = self.RCNN_base(im_data)
    prior = self.pen(base_feat)
    im_rec = self.rec(base_feat)
    ...
    recloss = nn.MSELoss()
    rec_loss = recloss(im_rec, im_data_down)


    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss, rec_loss
