...
class resnetDA_mchannel_mDA(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_imageDA_1 = _ImageDA(512)

    self.RCNN_top = nn.Sequential(resnet.layer4)

    self.relu = nn.ReLU(inplace=True)
    self.se1 = SELayer(512, 16)
    self.se2 = SELayer(self.dout_base_model, 16)
    self.downsample1 = nn.Sequential(nn.Conv2d(256, 512, kernel_size=1, stride=2, bias=False),
                                    nn.BatchNorm2d(512))
    self.downsample2 = nn.Sequential(nn.Conv2d(512, 1024, kernel_size=1, stride=2, bias=False),
                                    nn.BatchNorm2d(1024))
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    residual_1 = self.downsample1(base_feat_1)

    '''----------stage 2------------------------'''
    base_feat_2 = self.RCNN_base2(base_feat_1)
    base_feat_2_se = self.se1(base_feat_2)
    base_feat_2 = base_feat_2_se + residual_1
    base_feat_2 = self.relu(base_feat_2)
    residual_2 = self.downsample2(base_feat_2)

    '''----------stage 3------------------------'''
    base_feat = self.RCNN_base3(base_feat_2)
    base_feat_se = self.se2(base_feat)
    base_feat = base_feat_se + residual_2
    ...
    tgt_residual_1 = self.downsample1(tgt_base_feat_1)

    '''----------stage 2------------------------'''
    tgt_base_feat_2 = self.RCNN_base2(tgt_base_feat_1)
    tgt_base_feat_2_se = self.se1(tgt_base_feat_2)
    tgt_base_feat_2 = tgt_base_feat_2_se + tgt_residual_1
    tgt_base_feat_2 = self.relu(tgt_base_feat_2)
    tgt_residual_2 = self.downsample2(tgt_base_feat_2)

    '''----------stage 3------------------------'''
    tgt_base_feat = self.RCNN_base3(tgt_base_feat_2)
    tgt_base_feat_se = self.se2(tgt_base_feat)
    tgt_base_feat = tgt_base_feat_se + tgt_residual_2
    tgt_base_feat = self.relu(tgt_base_feat)
    ...
    base_score_1, base_label_1 = self.RCNN_imageDA_1(base_feat_2_se, need_backprop)
    ...
    tgt_base_score_1, tgt_base_label_1 = \
        self.RCNN_imageDA_1(tgt_base_feat_2_se, tgt_need_backprop)
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           ...
           DA_img_loss_cls_1,tgt_DA_img_loss_cls_1
