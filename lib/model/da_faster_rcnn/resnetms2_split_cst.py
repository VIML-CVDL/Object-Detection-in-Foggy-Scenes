...
class resnetms2_split_cst(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_base1 = nn.Sequential(resnet.conv1, resnet.bn1,resnet.relu,
      resnet.maxpool,resnet.layer1,resnet.layer2, resnet.layer3[:-7])
    self.RCNN_base2 = nn.Sequential(resnet.layer3[-7:])
    ...
    self.RCNN_imageDA_1 = _ImageDA(1024)
    self.RCNN_instanceDA = _InstanceDA(resnet=True)
    self.RCNN_instanceDA_1 = _InstanceDA(resnet=True)
    self.RCNN_roi_align_1 = RoIAlignAvg(cfg.POOLING_SIZE, cfg.POOLING_SIZE, 1.0/16.0)
    self.RCNN_rpn_1 = _RPN(self.dout_base_model)
    self.RCNN_proposal_target_1 = _ProposalTargetLayer(self.n_classes)
...
#    print('adasdasdadasdaasdasdasdasda')
    '''----------stage 1-----------------------'''
    ...
    self.RCNN_rpn.train()
    self.RCNN_rpn_1.train()
    rois_1, rpn_loss_cls_1, rpn_loss_bbox_1 = self.RCNN_rpn_1(base_feat_1, im_info, gt_boxes, num_boxes)
    rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(base_feat, im_info, gt_boxes, num_boxes)

    # if it is training phrase, then use ground trubut bboxes for refining
    if self.training:
        roi_data = self.RCNN_proposal_target(rois, gt_boxes, num_boxes)
        roi_data_1 = self.RCNN_proposal_target_1(rois_1, gt_boxes, num_boxes)
        rois, rois_label, rois_target, rois_inside_ws, rois_outside_ws = roi_data
        rois_1, rois_label_1, rois_target_1, rois_inside_ws_1, rois_outside_ws_1 = roi_data_1
        ...
        rois_label_1 = Variable(rois_label_1.view(-1).long())
        rois_target_1 = Variable(rois_target_1.view(-1, rois_target_1.size(2)))
        rois_inside_ws_1 = Variable(rois_inside_ws_1.view(-1, rois_inside_ws_1.size(2)))
        rois_outside_ws_1 = Variable(rois_outside_ws_1.view(-1, rois_outside_ws_1.size(2)))
    else:
        rois_label = None
        rois_target = None
        rois_inside_ws = None
    ...
    if cfg.POOLING_MODE == 'crop':
        ...
        pooled_feat_1 = self.RCNN_roi_align_1(base_feat_1, rois_1.view(-1, 5))
    ...
    if self.training and not self.class_agnostic:
        ...
        bbox_pred_select_1 = torch.gather(bbox_pred_view_1, 1, rois_label_1.view(rois_label_1.size(0), 1, 1).expand(rois_label_1.size(0), 1, 4))
    ...
    if self.training:
        ...
        # classification loss
        RCNN_loss_cls_1 = F.cross_entropy(cls_score_1, rois_label_1)
    ...
    # feed base feature map tp RPN to obtain rois
    self.RCNN_rpn.eval()
    self.RCNN_rpn_1.eval()
    tgt_rois_1, tgt_rpn_loss_cls_1, tgt_rpn_loss_bbox_1 = \
        self.RCNN_rpn_1(tgt_base_feat_1, tgt_im_info, tgt_gt_boxes, tgt_num_boxes)
    ...
    tgt_rois_1 = Variable(tgt_rois_1)
    # do roi pooling based on predicted rois
    ...
    if cfg.POOLING_MODE == 'crop':
        ...
        tgt_pooled_feat_1 = self.RCNN_roi_align_1(tgt_base_feat_1, tgt_rois_1.view(-1, 5))
    ...
    tgt_pooled_feat_1 = self._head_to_tail(tgt_pooled_feat_1)
    ...
    instance_sigmoid_1, same_size_label_1 = self.RCNN_instanceDA_1(pooled_feat_1, need_backprop)
    instance_loss_1 = nn.BCELoss()
    DA_ins_loss_cls_1 = instance_loss_1(instance_sigmoid_1, same_size_label_1)
    ...
    consistency_prob_1 = F.softmax(base_score_1, dim=1)[:,1,:,:]
    consistency_prob_1=torch.mean(consistency_prob_1)
    consistency_prob_1=consistency_prob_1.repeat(instance_sigmoid_1.size())

    DA_cst_loss_1=self.consistency_loss(instance_sigmoid_1,consistency_prob_1.detach())
    ...
    tgt_instance_sigmoid_1, tgt_same_size_label_1 = \
        self.RCNN_instanceDA_1(tgt_pooled_feat_1, tgt_need_backprop)
    tgt_instance_loss_1 = nn.BCELoss()

    tgt_DA_ins_loss_cls_1 = \
        tgt_instance_loss_1(tgt_instance_sigmoid_1, tgt_same_size_label)
    ...
    tgt_consistency_prob_1 = F.softmax(tgt_base_score_1, dim=1)[:, 0, :, :]
    tgt_consistency_prob_1 = torch.mean(tgt_consistency_prob_1)
    tgt_consistency_prob_1 = tgt_consistency_prob_1.repeat(tgt_instance_sigmoid_1.size())

    tgt_DA_cst_loss_1 = self.consistency_loss(tgt_instance_sigmoid_1, tgt_consistency_prob_1.detach())
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           rois_1, cls_prob_1, bbox_pred_1, rpn_loss_cls_1, rpn_loss_bbox_1, RCNN_loss_cls_1, RCNN_loss_bbox_1, rois_label_1,\
           DA_img_loss_cls,DA_ins_loss_cls,tgt_DA_img_loss_cls,tgt_DA_ins_loss_cls,DA_cst_loss,tgt_DA_cst_loss,\
           DA_img_loss_cls_1,DA_ins_loss_cls_1,tgt_DA_img_loss_cls_1,tgt_DA_ins_loss_cls_1,DA_cst_loss_1,tgt_DA_cst_loss_1
