...
class resnet_odonly(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.pen = PEN()
    for p in self.pen.parameters(): p.requires_grad=False
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    ploss = nn.MSELoss()
    prior_loss = ploss(tgt_prior, transmission_map)


    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, 0.1*prior_loss
