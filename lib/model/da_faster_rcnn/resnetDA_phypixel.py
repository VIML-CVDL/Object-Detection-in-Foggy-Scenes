...
class resnetDA_phypixel(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_imageDAres = _ImageDA(512)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    b, c, h, w = red.size()
    Rflat = torch.reshape(red, (b, 1, c, h*w))
    Gflat = torch.reshape(green, (b, 1, c, h*w))
    Bflat = torch.reshape(blue, (b, 1, c, h*w))

    # Use residual feature channel as 'transimission map' to guide pixcel-level DA
    ...
    res_feat = self.down(self.down(self.down(res_feat)))

    base_feat_1 = self.RCNN_base1(im_data)
    trans_feat = self.pen(base_feat_1)
    ...
    # feed image data to base model to obtain base feature map
    # feed image data to base model to obtain base feature map
    red_t = tgt_im_data[:, 0:1, :, :]
    green_t = tgt_im_data[:, 1:2, :, :]
    blue_t = tgt_im_data[:, 2:3, :, :]

    b, c, h, w = red.size()
    Rflat_t = torch.reshape(red_t, (b, 1, c, h*w))
    Gflat_t = torch.reshape(green_t, (b, 1, c, h*w))
    Bflat_t = torch.reshape(blue_t, (b, 1, c, h*w))

    # Use residual feature channel as 'transimission map' to guide pixcel-level DA
    stack_tensor_t = torch.cat((Rflat_t, Gflat_t, Bflat_t), dim=1)
    max_tensor_t, _ = torch.max(stack_tensor_t, 1)
    min_tensor_t, _ = torch.min(stack_tensor_t, 1)
    mul_max_feat_t = torch.reshape(max_tensor_t, (b, c, h, w))
    mul_min_feat_t = torch.reshape(min_tensor_t, (b, c, h, w))
    max_flat_t = torch.reshape(mul_max_feat_t, (b, 1, c, h*w))
    min_flat_t = torch.reshape(mul_min_feat_t, (b, 1, c, h*w))
    res_tensor_t = max_flat_t - min_flat_t
    tgt_res_feat = torch.reshape(res_tensor_t, (b, c, h, w))
    tgt_res_feat = self.down(self.down(self.down(tgt_res_feat)))

    tgt_base_feat_1 = self.RCNN_base1(tgt_im_data)
    tgt_trans_feat = self.pen(tgt_base_feat_1)
    ...
    priorloss = nn.MSELoss()
    prior_loss = priorloss(trans_feat, res_feat)/(h*w)
    tgt_prior_loss = priorloss(tgt_trans_feat, tgt_res_feat)/(h*w)

    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           DA_img_loss_cls,DA_ins_loss_cls,tgt_DA_img_loss_cls,tgt_DA_ins_loss_cls,DA_cst_loss,tgt_DA_cst_loss,\
           10*prior_loss, 10*tgt_prior_loss
