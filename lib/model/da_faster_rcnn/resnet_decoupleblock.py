...
class CLS(nn.Module):
    def __init__(self, n_classes):
        ...
        self.nn1 = nn.Linear(2048, 1024)
        self.bn1 = nn.BatchNorm1d(1024)
        self.nn2 = nn.Linear(1024, 256)
        self.bn2 = nn.BatchNorm1d(256)
        self.nn3 = nn.Linear(256, 128)
        self.bn3 = nn.BatchNorm1d(128)
        self.cls_score = nn.Linear(128, self.n_classes)
...
class resnet_decouple(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    resnet_cls = copy.deepcopy(resnet)

    self.RCNN_top = nn.Sequential(resnet.layer4)
    self.CLS = CLS(self.n_classes)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    cls_score_decoupled = self.CLS(pooled_feat.detach())
    cls_prob = F.softmax(cls_score, 1)
    cls_prob_decoupled = F.softmax(cls_score_decoupled, 1)

    RCNN_loss_cls = 0
    RCNN_loss_cls_decoupled = 0
    RCNN_loss_bbox = 0

    if self.training:
        # classification loss
        RCNN_loss_cls = F.cross_entropy(cls_score, rois_label)
        RCNN_loss_cls_decoupled = F.cross_entropy(cls_score_decoupled, rois_label)
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_cls_decoupled, RCNN_loss_bbox, rois_label
