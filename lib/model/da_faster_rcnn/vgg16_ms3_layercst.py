...
class vgg16_ms3_layercst(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    pooling = nn.MaxPool2d(2, stride=0)

    base_prob_1_pooled = pooling(base_prob_1)
    _,_,h,w = base_prob_1_pooled.size()
    DA_layer_loss_cls_1 = self.consistency_loss(base_prob_2,base_prob_1_pooled.detach())/(2*h*w)

    base_prob_2_pooled = pooling(base_prob_2)
    _,_,h,w = base_prob_2_pooled.size()
    DA_layer_loss_cls_2 = self.consistency_loss(base_prob_3,base_prob_2_pooled.detach())/(2*h*w)

    base_prob_3_pooled = pooling(base_prob_3)
    _,_,h,w = base_prob_3_pooled.size()
    DA_layer_loss_cls = self.consistency_loss(base_prob, base_prob_3_pooled.detach())/(2*h*w)
    ...
    tgt_base_prob_1_pooled = pooling(tgt_base_prob_1)
    _,_,h,w = tgt_base_prob_1_pooled.size()
    tgt_DA_layer_loss_cls_1 = self.consistency_loss(tgt_base_prob_2,tgt_base_prob_1_pooled.detach())/(2*h*w)

    tgt_base_prob_2_pooled = pooling(tgt_base_prob_2)
    _,_,h,w = tgt_base_prob_2_pooled.size()
    tgt_DA_layer_loss_cls_2 = self.consistency_loss(tgt_base_prob_3,tgt_base_prob_2_pooled.detach())/(2*h*w)

    tgt_base_prob_3_pooled = pooling(tgt_base_prob_3)
    _,_,h,w = tgt_base_prob_3_pooled.size()
    tgt_DA_layer_loss_cls = self.consistency_loss(tgt_base_prob, tgt_base_prob_3_pooled.detach())/(2*h*w)
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           ...
           DA_layer_loss_cls_2, DA_layer_loss_cls_1, DA_layer_loss_cls, tgt_DA_layer_loss_cls_2, tgt_DA_layer_loss_cls_1, \
           tgt_DA_layer_loss_cls
