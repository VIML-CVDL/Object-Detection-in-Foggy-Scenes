...
#im = Image.fromarray(src_in_trg.astype(np.uint8))
#im.save("fda.png")
import torch
...
def gaussian_basis_filters(scale, gpu, k=3):
    if gpu is not None:
        base = torch.log(torch.tensor(2.0).cuda())
        std = torch.exp(scale*base)
    else:
        base = torch.log(torch.tensor(2.0))
        std = torch.exp(scale*base)
    ...
    xsize = x.size()
    x1 = x.unsqueeze(1).repeat(1, xsize[0])
    x2 = torch.transpose(x.unsqueeze(1).repeat(1, xsize[0]),1,0)
    x=(x1,x2)
