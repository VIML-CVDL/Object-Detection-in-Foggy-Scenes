...
import pdb
from model.utils.net_utils import _smooth_l1_loss, _crop_pool_layer, _affine_grid_gen, _affine_theta, FocalLoss
from model.da_faster_rcnn.DA import _ImageDA, _ImageDAfocal
...
class vgg16_res2_add_2DA_mDA_ms_focalrgb(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_base1 = nn.Sequential(*list(vgg.features._modules.values())[:-15])
    self.RCNN_base2 = nn.Sequential(*list(vgg.features._modules.values())[-15:-1])

    self.Mul_MAX = Mul()
    self.Mul_MIN = Mul()
    self.Mul_CONTRAST = Mul()

    self.RCNN_imageDA =  _ImageDAfocal(self.dout_base_model)
    self.RCNN_imageDAres = _ImageDA(self.dout_base_model)
    self.RCNN_imageDA_1 = _ImageDAfocal(256)
    self.RCNN_imageDAres_1 = _ImageDA(256)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    '''----------stage 2------------------------'''
    base_feat_rgb = self.RCNN_base2(base_feat_rgb_1)
    res_feat = self.RCNN_base2(res_feat_1)
    ...
    '''----------stage 2------------------------'''
    tgt_base_feat_rgb = self.RCNN_base2(tgt_base_feat_rgb_1)
    tgt_res_feat = self.RCNN_base2(tgt_res_feat_1)
    ...
    base_score_1, base_label_1, focal_score_1 = self.RCNN_imageDA_1(base_feat_rgb_1, need_backprop, focal=True)
    base_score, base_label, focal_score = self.RCNN_imageDA(base_feat_rgb, need_backprop, focal=True)
    FL = FocalLoss(class_num=2, gamma=3)
    FL_1 = FocalLoss(class_num=2, gamma=3)
    # Image DA
    domain_s_1 = Variable(torch.ones(focal_score_1.size(0)).long().cuda())
    DA_img_focalloss_cls_1 = FL_1(focal_score_1, domain_s_1)
    domain_s = Variable(torch.ones(focal_score.size(0)).long().cuda())
    DA_img_focalloss_cls = FL(focal_score, domain_s)
    ...
    instance_sigmoid, same_size_label = self.RCNN_instanceDA(pooled_feat_rgb, need_backprop, withlogit=True)
    ...
    """  ************** taget loss ****************  """
    tgt_base_score_1, tgt_base_label_1, tgt_focal_score_1 = \
        self.RCNN_imageDA_1(tgt_base_feat_rgb_1, tgt_need_backprop, focal=True)
    tgt_base_score, tgt_base_label, tgt_focal_score = \
        self.RCNN_imageDA(tgt_base_feat_rgb, tgt_need_backprop, focal=True)

    tgt_FL_1 = FocalLoss(class_num=2, gamma=3)
    tgt_FL = FocalLoss(class_num=2, gamma=3)
    # Image DA
    domain_t_1 = Variable(torch.zeros(tgt_focal_score_1.size(0)).long().cuda())
    tgt_DA_img_focalloss_cls_1 = tgt_FL_1(tgt_focal_score_1, domain_t_1)
    domain_t = Variable(torch.zeros(tgt_focal_score.size(0)).long().cuda())
    tgt_DA_img_focalloss_cls = tgt_FL(tgt_focal_score, domain_t)
    ...
    tgt_instance_sigmoid, tgt_same_size_label = \
        self.RCNN_instanceDA(tgt_pooled_feat_rgb, tgt_need_backprop, withlogit=True)
    ...
    instance_sigmoid_res, same_size_label_res = self.RCNN_instanceDAres(pooled_res_feat, need_backprop, withlogit=True)
    instance_res_loss = nn.BCEWithLogitsLoss()
    ...
    tgt_instance_sigmoid_res, tgt_same_size_label_res = \
        self.RCNN_instanceDAres(tgt_pooled_res_feat, tgt_need_backprop, withlogit=True)
    tgt_instance_res_loss = nn.BCEWithLogitsLoss()
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
           ...
           tgt_DA_res_img_loss_cls_1, DA_img_focalloss_cls, DA_img_focalloss_cls_1, tgt_DA_img_focalloss_cls, tgt_DA_img_focalloss_cls_1
