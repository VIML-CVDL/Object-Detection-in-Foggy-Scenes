...
class resnetDA_transmission_ear(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    base_feat = self.RCNN_base[:6](im_data)
    prior = self.pen(base_feat)
    base_feat = self.RCNN_base[6](base_feat)
    ...
    # feed image data to base model to obtain base feature map
    tgt_base_feat = self.RCNN_base[:6](tgt_im_data)
    tgt_prior = self.pen(tgt_base_feat)
    tgt_base_feat = self.RCNN_base[6](tgt_base_feat)
