...
class resnetDA_scl_os(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
            tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes, tgt_need_backprop, transmission_map=None, tgt_transmission_map=None):
    ...
    d_inst = self.netD_inst(grad_reverse(pooled_feat))
    # compute bbox offset
    ...
    tgt_domain_p1 = self.netD1(grad_reverse(tgt_base_feat1))
    tgt_feat1 = self.netD_forward1(tgt_base_feat1.detach())

    tgt_base_feat2 = self.RCNN_base2(tgt_base_feat1)

    tgt_domain_p2 = self.netD2(grad_reverse(tgt_base_feat2))
    tgt_feat2 = self.netD_forward2(tgt_base_feat2.detach())

    tgt_base_feat = self.RCNN_base3(tgt_base_feat2)

    tgt_domain_p3 = self.netD3(grad_reverse(tgt_base_feat))
    tgt_feat3 = self.netD_forward3(tgt_base_feat.detach())
    ...
    tgt_feat1 = tgt_feat1.view(1, -1).repeat(tgt_pooled_feat.size(0), 1)
    tgt_pooled_feat = torch.cat((tgt_feat1, tgt_pooled_feat), 1)

    tgt_feat2 = tgt_feat2.view(1, -1).repeat(tgt_pooled_feat.size(0), 1)
    tgt_pooled_feat = torch.cat((tgt_feat2, tgt_pooled_feat), 1)


    tgt_feat3 = tgt_feat3.view(1, -1).repeat(tgt_pooled_feat.size(0), 1)
    tgt_pooled_feat = torch.cat((tgt_feat3, tgt_pooled_feat), 1)

    tgt_d_inst = self.netD_inst(grad_reverse(tgt_pooled_feat))

    """===================== for DA ================================"""
    FL = FocalLoss(class_num=2)
    "source"
    out_d_inst, out_d1, out_d2, out_d3 = d_inst, domain_p1, domain_p2, domain_p3
    ...
    # k=2nd loss
    dloss_s2 = 0.5 * F.cross_entropy(out_d2, domain_s2) * 0.15
    # k = 3rd loss
    ...
    "target"
    tgt_out_d_inst, tgt_out_d1, tgt_out_d2, tgt_out_d3 = tgt_d_inst, tgt_domain_p1, tgt_domain_p2, tgt_domain_p3

    # domain label
    domain_t2 = domain_t3 = Variable(torch.ones(tgt_out_d2.size(0)).long().cuda())
    domain_t_p = Variable(torch.ones(tgt_out_d_inst.size(0)).long().cuda())
    # k=1th loss
    dloss_t1 = 0.5 * torch.mean((1 - tgt_out_d1) ** 2)
    # k=2nd loss
    dloss_t2 = 0.5 * F.cross_entropy(tgt_out_d2, domain_t2) * 0.15
    # k = 3rd loss
    dloss_t3 = 0.5 * FL(tgt_out_d3, domain_t3)
    # instance alignment loss
    dloss_t_p = 0.5 * FL(tgt_out_d_inst, domain_t_p)

    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, dloss_s1, dloss_t1, dloss_s2, dloss_t2, dloss_s3, dloss_t3, dloss_s_p, dloss_t_p
