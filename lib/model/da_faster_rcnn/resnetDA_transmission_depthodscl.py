...
class netD_forward(nn.Module):
    def __init__(self):
        super(netD_forward, self).__init__()
...
class resnetDA_transmission_depthodscl(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_cls_score = nn.Linear(2048+128, self.n_classes)
    if self.class_agnostic:
      self.RCNN_bbox_pred = nn.Linear(2048+128, 4)
    else:
      self.RCNN_bbox_pred = nn.Linear(2048+128, 4 * self.n_classes)

    self.pen = PEN()
    self.den = DEN()
    self.netD_forward = netD_forward()
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    feat = self.netD_forward(dep.detach())
