...
class PEN(nn.Module):
    def __init__(self,dim):
        ...
        self.dim = dim
        self.conv1 = conv1x1(self.dim, 128)
...
class resnetDA_transmission_ms3(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
            tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes, tgt_need_backprop, transmission_map=None, tgt_transmission_map=None, transmission_map_ms=None, tgt_transmission_map_ms=None\
            , transmission_map_ms3=None, tgt_transmission_map_ms3=None):
    ...
    prior1 = self.pen1(base_feat_1)
    prior2 = self.pen2(base_feat_2)
    prior3 = self.pen3(base_feat)
    ...
    tgt_prior1 = self.pen1(tgt_base_feat_1)
    tgt_prior2 = self.pen2(tgt_base_feat_2)
    tgt_prior3 = self.pen3(tgt_base_feat)
    ...
    ploss = nn.MSELoss()
    prior_loss = ploss(prior1, transmission_map_ms3)/16
    prior_loss += ploss(tgt_prior1, tgt_transmission_map_ms3.squeeze(0).permute([0,3,1,2]))/16
    prior_loss += ploss(prior2, transmission_map_ms)/4
    prior_loss += ploss(tgt_prior2, tgt_transmission_map_ms.squeeze(0).permute([0,3,1,2]))/4
    prior_loss += ploss(prior3, transmission_map)
    prior_loss += ploss(tgt_prior3, tgt_transmission_map.squeeze(0).permute([0,3,1,2]))
