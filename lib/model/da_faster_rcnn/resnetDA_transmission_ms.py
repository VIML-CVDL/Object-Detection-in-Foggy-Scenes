...
class resnetDA_transmission_ms(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    tgt_prior2 = self.pen2(tgt_base_feat)
    ...
    ploss = nn.MSELoss()
    prior_loss = ploss(prior2, transmission_map)
    prior_loss += ploss(tgt_prior2, tgt_transmission_map.squeeze(0).permute([0,3,1,2]))
    prior_loss += ploss(prior1, transmission_map_ms)
    prior_loss += ploss(tgt_prior1, tgt_transmission_map_ms.squeeze(0).permute([0,3,1,2]))
