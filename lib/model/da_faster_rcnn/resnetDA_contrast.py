...
class resnetDA_contrast(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    G_x=self.conv1(tgt_base_feat)
    G_y=self.conv2(tgt_base_feat)

    g_sum = (torch.sum(G_x)+torch.sum(G_y))/(1024*38*75)
