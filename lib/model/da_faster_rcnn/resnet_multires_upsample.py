...
class resnet_multires_upsample(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.down = Down(1024, 512)
    self.fuse = Fuse(1024, 1024)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    _, _, h, w = base_feat_1.size()
    base_feat_up = F.upsample(input=base_feat, size=(h,w), mode='bilinear', align_corners=True)
    base_feat_up = self.down(base_feat_up)
    base_feat_1 = torch.cat((base_feat_up, base_feat_1), 1)
    base_feat_1 = self.fuse(base_feat_1)
    ...
    if cfg.POOLING_MODE == 'crop':
        ...
        pooled_feat_1 = self.RCNN_roi_align_1(base_feat_1, rois.view(-1, 5))
    ...
    _, _, h, w = tgt_base_feat_1.size()
    tgt_base_feat_up = F.upsample(input=tgt_base_feat, size=(h,w), mode='bilinear', align_corners=True)
    tgt_base_feat_up = self.down(tgt_base_feat_up)
    tgt_base_feat_1 = torch.cat((tgt_base_feat_up, tgt_base_feat_1), 1)
    tgt_base_feat_1 = self.fuse(tgt_base_feat_1)
    ...
    tgt_rois_1, tgt_rpn_loss_cls_1, tgt_rpn_loss_bbox_1 = self.RCNN_rpn_1(tgt_base_feat_1, tgt_im_info, tgt_gt_boxes, tgt_num_boxes)
    tgt_rois = torch.cat((tgt_rois, tgt_rois_1),1)
    ...
    if cfg.POOLING_MODE == 'crop':
        ...
        tgt_pooled_feat_1 = self.RCNN_roi_align(tgt_base_feat_1, tgt_rois.view(-1, 5))
    elif cfg.POOLING_MODE == 'pool':
    ...
    tgt_pooled_feat = torch.cat((tgt_pooled_feat, tgt_pooled_feat_1),1)
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, rpn_loss_cls_1, rpn_loss_bbox_1, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
