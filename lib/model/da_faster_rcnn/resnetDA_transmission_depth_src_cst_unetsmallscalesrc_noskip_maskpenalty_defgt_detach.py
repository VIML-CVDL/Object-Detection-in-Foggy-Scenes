...
class resnetDA_transmission_depth_src_cst_unetsmallscalesrc_noskip_maskpenalty_defgt_detach(_fasterRCNN):
...
#                                                up_conv_in_channels=128, up_conv_out_channels=64))
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    obj_mask = torch.ones_like(im_data)
    _, number_obj, _ = gt_boxes.size()
    obj_list = gt_boxes[0]
    ...
    rev_base_feat = grad_reverse(base_feat)
    up_1 = self.up_blocks[0](rev_base_feat)
    up_2 = self.up_blocks[1](up_1)
    rec_feat = self.up_blocks[2](up_2)


    def_mask = torch.ones_like(depth) - depth
    _, _, rec_h, rec_w = rec_feat.size()
    def_mask = torch.nn.Upsample(size=(rec_h, rec_w), mode='bilinear')(def_mask)

    _, _, rec_h, rec_w = rec_feat.size()
    im_data = torch.nn.Upsample(size=(rec_h, rec_w), mode='bilinear')(im_data)
    im_data_new = (im_data-torch.min(im_data))/(torch.max(im_data)-torch.min(im_data))


    obj_mask = torch.nn.Upsample(size=(rec_h, rec_w), mode='bilinear')(obj_mask)
    ...
    recloss = nn.MSELoss()
    rec_loss = recloss(def_mask.detach()*obj_mask*rec_feat, def_mask.detach()*obj_mask*im_data_new)
    #rec_loss = 0
