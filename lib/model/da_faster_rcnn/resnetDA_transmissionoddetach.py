...
class resnetDA_transmissionoddetach(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    prior_transfer = prior[:,0,:,:].view(1, -1).repeat(pooled_feat.size(0), 1)
    prior_transfer = self.transfer(prior_transfer.detach())
