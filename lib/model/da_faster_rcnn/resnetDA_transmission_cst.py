...
class resnetDA_transmission_cst(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    consistency_prob = F.softmax(prior, dim=1)[:,1,:,:]
    ...
    tgt_consistency_prob = F.softmax(tgt_prior, dim=1)[:, 0, :, :]
    ...
    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss, DA_ins_loss_cls, tgt_DA_ins_loss_cls, DA_cst_loss, tgt_DA_cst_loss
