...
class resnetDA_transmission_depth_src_cst_edgeenh_earxy(_fasterRCNN):
  ...
  def grad_conv(self, x):

    b,c,h,w = x.size()
    k1 = np.array([[[[1,0,-1],[2,0,-2],[1,0,-1]]]])
    k2 = np.array([[[[1,2,1],[0,0,0],[-1,-2,-1]]]])
    kernal1 = torch.from_numpy(k1).float().cuda()
    kernal2 = torch.from_numpy(k2).float().cuda()
    delta_x = F.conv2d(x[:,0:1,:,:],kernal1,padding=1)
    delta_y = F.conv2d(x[:,0:1,:,:],kernal2,padding=1)

    return torch.sqrt(torch.pow(delta_x, 2) + torch.pow(delta_y, 2) + 1e-7)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    base_feat1 = self.RCNN_base[:6](im_data)
    base_feat = self.RCNN_base[6](base_feat1)
    ...
    # feed image data to base model to obtain base feature map
    tgt_base_feat1 = self.RCNN_base[:6](tgt_im_data)
    tgt_base_feat = self.RCNN_base[6](tgt_base_feat1)
    ...
    img_clean_edge = self.grad_conv(base_feat1)
    img_hazy_edge = self.grad_conv(tgt_base_feat1)
    edge_loss = torch.abs(1 - (img_clean_edge*img_hazy_edge).sum() / (img_hazy_edge.sum()*10))

    return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, prior_loss, depth_loss, cst_loss, edge_loss
