...
class SE(nn.Module):
    def __init__(self, in_channels, out_channels, context=False):
        ...
        self.conv1 = conv1x1(in_channels, int(in_channels/16))
        self.conv2 = conv1x1(int(in_channels/16), out_channels)
        self.context = context
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                nn.init.kaiming_normal_(m.weight.data, mode='fan_in')
...
class vgg16_ms3_concat_1by1_se(_fasterRCNN):
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    tgt_base_feat = self.se(tgt_base_feat)
