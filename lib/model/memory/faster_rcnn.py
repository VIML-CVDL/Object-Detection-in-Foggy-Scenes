...
from model.memory.memDA import _ImageDA
import time
import pdb
...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        for i in range(0, self.n_classes-1):
            mem = Memory(memory_size=20, feature_dim=512, key_dim=512).cuda()
            self.memory_list.append(mem)
            cda = _ImageDA(self.dout_base_model).cuda()
            self.cda_list.append(cda)


    def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
                tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes, tgt_need_backprop, keys_list):
        ...
        '''l2loss = nn.MSELoss()
        DA_img_loss_cls = l2loss(base_score,torch.ones_like(base_score))'''
        ...
        #tgt_DA_img_loss_cls = l2loss(tgt_base_score,torch.zeros_like(base_score))

        """  ************** category-wise DA loss ****************  """
        gathering_loss_sum = None
        spreading_loss_sum = None
        cda_loss_sum = None

        updated_memory_list = []
        #print(im_info[0][0].item())
        #print(torch.zeros((im_info[0][0], im_info[0][1])).cuda())
        for i in range(0, self.n_classes-1):
            indices = (gt_boxes[:,:,4]==i+1).nonzero().tolist()
            # update
            updated_memory, cfeature, gathering_loss, spreading_loss = self.memory_list[i](base_feat, keys_list[i], True, gt_boxes, indices)

            base_score_cda, base_label_cda = self.cda_list[i](cfeature, need_backprop)
            base_prob_cda = F.log_softmax(base_score_cda, dim=1)
            DA_img_loss_cls_cda = F.nll_loss(base_prob_cda, base_label_cda)
            #DA_img_loss_cls_cda = l2loss(base_score_cda,torch.ones_like(base_score_cda))


            updated_memory, tgt_cfeature = self.memory_list[i](tgt_base_feat, keys_list[i], False)
            tgt_base_score_cda, tgt_base_label_cda = \
                self.cda_list[i](tgt_cfeature, tgt_need_backprop)
            tgt_base_prob_cda = F.log_softmax(tgt_base_score_cda, dim=1)
            tgt_DA_img_loss_cls_cda = F.nll_loss(tgt_base_prob_cda, tgt_base_label_cda)

            #tgt_DA_img_loss_cls_cda = l2loss(tgt_base_score_cda,torch.zeros_like(base_score_cda))
            cda_loss = DA_img_loss_cls_cda + tgt_DA_img_loss_cls_cda
            if cda_loss_sum is None:
                cda_loss_sum = cda_loss
            else:
                cda_loss_sum += cda_loss

            if gathering_loss_sum is None:
                gathering_loss_sum = gathering_loss
            else:
                gathering_loss_sum += gathering_loss

            if spreading_loss_sum is None:
                spreading_loss_sum = spreading_loss
            else:
                spreading_loss_sum += spreading_loss

            updated_memory_list.append(updated_memory)

        return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,\
               DA_img_loss_cls,tgt_DA_img_loss_cls, gathering_loss_sum, spreading_loss_sum, updated_memory_list, cda_loss_sum
