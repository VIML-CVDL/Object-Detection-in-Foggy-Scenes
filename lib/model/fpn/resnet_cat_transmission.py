...
class resnet_fpn(_FPN):
  ...
  def _init_modules(self):
    ...
    # Smooth layers
    self.RCNN_smooth1 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth2 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
    self.RCNN_smooth3 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
  ...
  def _upsample_cat(self, x, y):
    ...
    '''
    _,_,H,W = y.size()
    return torch.cat((F.upsample(x, size=(H,W), mode='bilinear'),y),1)
  ...
  def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
    ...
    p4 = self._upsample_cat(p5, self.RCNN_latlayer1(c4.detach()))
    p4 = self.RCNN_smooth1(p4)
    p3 = self._upsample_cat(p4, self.RCNN_latlayer2(c3.detach()))
    p3 = self.RCNN_smooth2(p3)
    p2 = self._upsample_cat(p3, self.RCNN_latlayer3(c2.detach()))
    ...
    tgt_p4 = self._upsample_cat(tgt_p5, self.RCNN_latlayer1(tgt_c4.detach()))
    tgt_p4 = self.RCNN_smooth1(tgt_p4)
    tgt_p3 = self._upsample_cat(tgt_p4, self.RCNN_latlayer2(tgt_c3.detach()))
    tgt_p3 = self.RCNN_smooth2(tgt_p3)
    tgt_p2 = self._upsample_cat(tgt_p3, self.RCNN_latlayer3(tgt_c2.detach()))
