...
def prepare_roidb(imdb):
  ...
  for i in range(len(imdb.image_index)):
    ...
    tmp_path = imdb.image_path_at(i).split('/')
    prior_path = ''
    for j in range(len(tmp_path) - 2):
        prior_path += tmp_path[j] + '/'

    if 'leftImg8bit' in imdb.image_path_at(i):
        roidb[i]['transmission'] = #{transmission map path}
        head, tail =str(tmp_path[-1]).split('leftImg8bit')
        roidb[i]['depth'] = #{depth map path}
        roidb[i]['grad'] = #{gradient map path}, not needed in this project
        roidb[i]['clean'] = #{defogged image path}
    else:
        roidb[i]['transmission'] = #{transmission map path}
        roidb[i]['depth'] = #{depth map path}
        roidb[i]['grad'] = #{gradient map path}, not needed in this project
